...
public class Contacts implements Serializable {
    ...
    //uus Contacts muudetud võrreldes eelmisega
    public Contacts(SQLiteDatabase sqLiteDatabase, String firstn, String lastn, String addres, String mail) {
        try {
            Cursor cursor= sqLiteDatabase.query(DatabaseHelper.CONTACTS_TABLE, DatabaseHelper.CONTACTS_COLUMNS,
                    "first like ? and last like ? and address like? and email like ?",
                    new String[] {firstn + "%", lastn + "%",  "%"+ addres +"%", "%"+ mail +"%"},
                    null, null, null);
            for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()){
                int id= cursor.getInt(DatabaseHelper.ID_COLUMN);
                String first= cursor.getString(DatabaseHelper.FIRST_COLUMN);
    ...
    private Code getCode(SQLiteDatabase sqLiteDatabase, String code) {
        Cursor cursor =sqLiteDatabase.query(DatabaseHelper.COUNTRY_TABLE, DatabaseHelper.COUNTRY_COLUMNS,
